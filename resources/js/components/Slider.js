...
class Slider extends Component {
    ...
    onDelete(contact_id){
        axios.delete('/api/slider/'+contact_id, {
            headers: {
                                            ...
                                            {                                                
                                                this.state.contact.map((row, index)=> {
                                                    return (<tr>
                                                        ...
                                                        <td>{row.title}</td>
                                                        <td><Link exact to={"/author/slider/edit/" + row.id}><i className="fa fa-pencil-square" aria-hidden="true"></i></Link> | <a onClick={this.onDelete.bind(this,row.id)} href="javascript:void(0)"><i className="fa fa-trash-o" aria-hidden="true"></i></a></td>
                                                    </tr>)
                                                })
                                            }
                                        </tbody>
